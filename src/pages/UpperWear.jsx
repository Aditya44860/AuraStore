    ...
    const fetchProducts = async () => {
      ...
      try {
        const url = selectedFilter === 'all'
          ? `${import.meta.env.VITE_API_BASE_URL}/api/products/category/Upper Wear?page=${currentPage}&limit=${itemsPerPage}&sortBy=${sortBy}`
          : `${import.meta.env.VITE_API_BASE_URL}/api/products/category/Upper Wear?subcategory=${selectedFilter}&page=${currentPage}&limit=${itemsPerPage}&sortBy=${sortBy}`
      ...
      <div className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
          <div className="text-center mb-10 sm:mb-14">
            <h1 className="text-2xl sm:text-3xl lg:text-[2.5rem] font-extralight text-gray-900 mb-2 sm:mb-3 tracking-tight">Upper Wear</h1>
            <p className="text-gray-500 text-sm sm:text-[15px] font-medium mb-8">Hoodies, T-Shirts, Jackets & More</p>
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
              {filterOrder.map((filter) => (
                ...
                >
                  {filter === 'tshirts' ? 'T-Shirts' : filter}
                </button>
              ))}
            </div>
          </div>
